...
public class SingleValue<T>
{
    ...
    public SynchedEntityData.DataValue<T> wrap(T value)
    {
        if (customSerializeMethod != null)
            return (SynchedEntityData.DataValue<T>) customSerializeMethod.apply(this, value);

        var defaultSerializer = (net.minecraft.network.syncher.EntityDataSerializer<T>) ProtocolRegistryUtils.getSerializer(this);
        if (defaultSerializer == null)
            throw new NullDependencyException("No serializer available for '%s', cannot wrap its value!".formatted(name));

        return new SynchedEntityData.DataValue<>(this.index, defaultSerializer, value);
    }
