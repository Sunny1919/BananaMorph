...
public class WardenWatcher extends EHasAttackAnimationWatcher
{
    ...
    protected <X> void onEntryWrite(CustomEntry<X> entry, X oldVal, X newVal)
    {
        ...
        if (entry.equals(CustomEntries.WARDEN_CHARGING_ATTACK) && Boolean.TRUE.equals(newVal))
        {
            ...
            sendPacketToAffectedPlayers(new ClientboundEntityEventPacket(entity, (byte)62));
        }
        ...
        if (entry.equals(CustomEntries.ANIMATION))
        {
            ...
            switch (id)
            {
                ...
                case AnimationNames.APPEAR ->
                {
                    ...
                    var despawnPacket = new ClientboundRemoveEntitiesPacket(bindingPlayer.getEntityId());

                    for (Player affectedPlayer : affectedPlayers)
                    {
                        var nmsPlayer = NmsRecord.ofPlayer(affectedPlayer);
                        if (nmsPlayer.connection != null)
                        {
                            nmsPlayer.connection.sendPacket(despawnPacket);
                            packets.forEach(p -> nmsPlayer.connection.sendPacket(p));
                        }
                    }
                }
