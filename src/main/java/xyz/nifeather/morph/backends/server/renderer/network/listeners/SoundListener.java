...
public class SoundListener extends ProtocolListener
{
    ...
    {
        if (event.getNMSPacket() instanceof ClientboundSoundPacket clientboundSoundPacket)
        {
            //不要处理来自我们自己的包
            if (getFactory().isPacketOurs(clientboundSoundPacket))
            {
                return;
            }

            var newHandler = getClientboundSoundPacket(event, clientboundSoundPacket);
            if (newHandler == null) return;

            getFactory().markPacketOurs(newHandler);
            event.setNMSPacket(newHandler);
        }
    }
    ...
    @Nullable
    private ClientboundSoundPacket getClientboundSoundPacket(PacketSendEvent event, ClientboundSoundPacket handle)
    {
